...
export default function ExpiryForm({
  ...
  useEffect(() => {
    ...
    const hasItem = barcode.trim() || stock_id.trim() || description.trim();
  ...
  }, [dupBarcode, dupStockId, dupDescription, dupExpiryDate, isOpen, editingEntry, isManager]);
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto px-6 py-5 space-y-4 flex-1">
          ...
          <div>
            ...
            <input
              ...
              onFocus={(e) => {
                e.currentTarget.style.borderColor = dupStatus === "exact" ? "#ef4444" : "#2563eb";
                e.currentTarget.style.boxShadow = "0 0 0 2px rgba(37,99,235,0.05)";
              }}
              onBlur={(e) => {
            ...
            {dupStatus === "exact" && dupInfo && (
              <div className="mt-2 rounded-xl border border-red-200 bg-red-50 p-3.5 space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm font-semibold text-[#ef4444]">Duplicate Entry Detected</p>
                </div>
                ...
                <p className="text-xs font-semibold text-[#ef4444]">Cannot submit duplicate entry.</p>
              </div>
            )}
