...
const ACTION_BADGE: Record<string, { label: string; cls: string }> = {
  CREATE: { label: "🟢 Create", cls: "bg-green-500/15 text-green-400 border border-green-500/20" },
  UPDATE: { label: "🟡 Update", cls: "bg-yellow-500/15 text-yellow-400 border border-yellow-500/20" },
  DELETE: { label: "🔴 Delete", cls: "bg-red-500/15 text-red-400 border border-red-500/20" },
};
...
export default function HistoryTable({ entries, isLoading, total, page, totalPages, setPage }: Props) {
  if (isLoading) {
    return (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-900 border-b border-gray-800 text-left">
                <th className="px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider whitespace-nowrap">Timestamp</th>
                <th className="px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">PIC</th>
                <th className="px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">Action</th>
                <th className="px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">Module</th>
            ...
            <tbody className="divide-y divide-gray-800/60">
              {groups.map((g) => (
                <>
                  ...
                  {g.rows.map((entry) => {
                    const action = ACTION_BADGE[entry.action] ?? { label: entry.action, cls: "bg-gray-800 text-gray-400 border border-gray-700" };
                    const moduleLbl = MODULE_BADGE[entry.module] ?? entry.module;
                    return (
                      <tr key={entry.id} className="hover:bg-gray-900/30 transition-colors">
                        ...
                        <td className="px-4 py-3 text-white whitespace-nowrap">
                          {entry.pic_name ?? <span className="text-gray-600">—</span>}
                        </td>
                        <td className="px-4 py-3">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium ${action.cls}`}>
                            {action.label}
                          </span>
                        </td>
                        <td className="px-4 py-3">
