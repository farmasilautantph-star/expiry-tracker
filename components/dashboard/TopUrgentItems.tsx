...
const URGENCY = {
  ...
  critical: { label: "",         cls: "bg-orange-500/20 text-orange-400 border border-orange-500/30" },
  warning:  { label: "",         cls: "bg-yellow-500/20 text-yellow-400 border border-yellow-500/30" },
};
  ...
  return (
    <div className="flex flex-col divide-y divide-gray-800/60 overflow-y-auto max-h-[300px]">
      {items.map((item) => {
        ...
        const daysLabel = item.days_left <= 0
          ? "Expired"
        ...
        return (
          ...
          >
            <span className={`mt-0.5 inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium whitespace-nowrap flex-shrink-0 ${u.cls}`}>
