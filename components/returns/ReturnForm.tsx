...
import type { ReturnEntry, ReturnFormData } from "@/hooks/useReturns";
...
export default function ReturnForm({ isOpen, onClose, picName, editingEntry, onSubmit }: Props) {
  const [form, setForm] = useState<ReturnFormData>(EMPTY);
  ...
  useEffect(() => {
    ...
    if (editingEntry) {
      setForm({
        logged_date:    editingEntry.logged_date.split("T")[0],
      ...
      setForm({ ...EMPTY, logged_date: todayString() });
    }
    setError("");
    setResults([]);
    setActiveField(null);
  ...
  function set<K extends keyof ReturnFormData>(field: K, value: ReturnFormData[K]) {
    setForm((prev) => ({ ...prev, [field]: value }));
  }

  function triggerSearch(value: string, field: SearchField) {
    if (debounceRef.current) clearTimeout(debounceRef.current);
    if (value.length < 2) { setResults([]); setActiveField(null); return; }
  ...
  function selectProduct(p: ProductResult) {
    setForm((prev) => ({
      ...prev,
      stock_id:    p.stock_id    ?? prev.stock_id,
      barcode:     p.barcode     ?? prev.barcode,
      description: p.description ?? prev.description,
      category:    p.category_id ?? prev.category,
      uom:         p.uom         ?? prev.uom,
    }));
    setResults([]);
    setActiveField(null);
  ...
  async function handleSubmit(e: FormEvent) {
    e.preventDefault();
    setError("");
    if (!form.logged_date)        { setError("Date is required."); return; }
    if (!form.barcode.trim())     { setError("Barcode is required."); return; }
    if (!form.description.trim()) { setError("Description is required."); return; }
    if (!form.category.trim())    { setError("Category is required."); return; }
    if (!form.return_by_date)     { setError("Return By Date is required."); return; }
  ...
  function SearchDropdown() {
    ...
    return (
      <div className="absolute z-50 left-0 right-0 top-full mt-1 bg-gray-800 border border-gray-700 rounded-lg shadow-xl overflow-hidden max-h-52 overflow-y-auto">
        {isSearching && <div className="px-3 py-2 text-xs text-gray-500">Searching…</div>}
        {results.map((p) => (
          <button
            key={p.id}
            type="button"
      ...
      <div className="relative w-full max-w-lg bg-gray-900 border border-gray-800 rounded-2xl shadow-2xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800 flex-shrink-0">
          <h2 className="text-base font-semibold text-white">
            {editingEntry ? "Edit Return Entry" : "Log Return"}
          </h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto px-6 py-5 space-y-4 flex-1">
          ...
          {/* Date */}
          <div>
            <Label required>Date Returned</Label>
            <input
              type="date"
          ...
          <div className="flex items-center justify-end gap-3 pt-1 pb-1">
            ...
            >
              {submitting ? "Saving…" : editingEntry ? "Save Changes" : "Log Return"}
            </button>
          </div>
        </form>
      </div>
