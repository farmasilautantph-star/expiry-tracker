...
export async function PUT(
  ...
  if (existing.return_status !== "pending" && existing.return_status !== "returned") {
    return NextResponse.json({ success: false, error: "Entry is not in the return list" }, { status: 400 });
  }
  ...
  if (user.role !== "manager") {
    ...
    db.prepare("UPDATE expiry_logs SET return_status = 'returned' WHERE id = ?").run(id);
    db.prepare(
      "INSERT INTO history_log (action, module, record_id, pic_id, pic_name, description, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?)"
    ).run("UPDATE", "returns", id, user.userId, user.picName,
      `Marked returned: ${existing.description}`, now);
    const updated = db.prepare("SELECT * FROM expiry_logs WHERE id = ?").get(id) as unknown as ExpiryRow;
  ...
  db.prepare(
    ...
    "UPDATE", "returns", id, user.userId, user.picName,
