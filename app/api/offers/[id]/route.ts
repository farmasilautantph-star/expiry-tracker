...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (offer_status !== undefined && validStatus.includes(offer_status)) {
    fields.push("offer_status = ?"); bindings.push(offer_status);
  }
  if (quantity !== undefined) {
    fields.push("quantity = ?"); bindings.push(Math.max(1, Number(quantity)));
  }
  if (outlet_name !== undefined) {
    fields.push("outlet_name = ?"); bindings.push(outlet_name?.trim() || "");
  }
  if (has_alert !== undefined) {
    fields.push("has_alert = ?"); bindings.push(has_alert ? 1 : 0);
  }
  if (notes !== undefined) {
    fields.push("notes = ?"); bindings.push(notes?.trim() || null);
  }
  ...
  ).run("UPDATE", "offers", id, user.userId, user.picName, `Updated offer id=${id}`, now);
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const db = getDb();
  const existing = db.prepare("SELECT * FROM offers WHERE id = ?").get(id) as { description?: string; outlet_name?: string } | undefined;
  if (!existing) return NextResponse.json({ success: false, error: "Not found" }, { status: 404 });
  ...
  db.prepare(
    "INSERT INTO history_log (action, module, record_id, pic_id, pic_name, description, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?)"
  ).run("DELETE", "offers", id, user.userId, user.picName,
    `Deleted offer: ${existing.description ?? ""} → ${existing.outlet_name ?? ""}`, now);
